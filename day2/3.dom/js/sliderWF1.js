...
(function ($) {
    ...
    SliderWF.prototype = {
        sliderInit: function () {
            ...
            $(this.rootTag + " .sliderWF " + this.prevTagClassOrId).click(this.sliderRight);
            $(this.rootTag + " .sliderWF " + this.nextTagClassOrId).click(this.sliderLeft);

        },

        sliderLeft: function (e) {
            //滑块左移
            e.stopPropagation();
            //判断是否正在动画 如果正在动画则不继续
        ...
        //用于指定millisec后执行cb
        sliderOperationByTime: function (millisec, cb) {
            var callback=sliderLeft;
            if (Object.prototype.toString.call(val) == "[object Function]") {
                callback=cb;
            }
            setTimeout(millisec,callback());
            this.returnIfAnimate();
        }
