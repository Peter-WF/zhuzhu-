...
(function ($) {
    ...
    function SliderWF(data) {
        ...
        this.viewpadding = data.viewpadding ? data.viewpadding : 0; //padding左右间隔 考虑废弃的属性
        ...
        this.totalTagClassOrId = data.totalTagClassOrId ? data.totalTagClassOrId : ".sliderWF-total"; //绑定总滑块数
        this.currentIndexTagClassOrId = data.currentIndexTagClassOrId ? data.currentIndexTagClassOrId : ".sliderWF-currentIndex"; //绑定当前滑块序号tag
        this.marginRightW= data.marginRightW ? data.marginRightW : 0; //滑块左外边距
        this.marginLeftW= data.marginLeftW ? data.marginLeftW : 0; //滑块右外边距
        ...
        this.viewwidth = (this.viewwidthTotal.substr(0, this.viewwidthTotal.length - 2) -(this.nView-1)*(this.marginLeftW+this.marginRightW))/ this.nView + "px"; //每个展示项的宽度
    ...
    SliderWF.prototype = {
        sliderInit: function () {
            $(this.prevTagClassOrId).hide(); //隐藏左滑块按钮
            $(this.totalTagClassOrId).html(this.nImg);
            ...
            $(this.rootTag + " .sliderWF-main").css("width", (this.viewwidth.substr(0, this.viewwidth.length - 2)-0+this.marginLeftW+this.marginRightW)* this.nImg);
            if (SliderWF[this.rootTag]) {
                $(this.prevTagClassOrId).off("click");
                $(this.nextTagClassOrId).off("click");
            ...
            $(this.prevTagClassOrId).click(function (e) {
                //滑块右移
                e.stopPropagation();
            ...
            $(this.nextTagClassOrId).click(function (e) {
                //滑块左移
                e.stopPropagation();
                //判断是否正在动画 如果正在动画则不继续
                if (_this_.currentSliderIndex < _this_.nImg - 1 * _this_.nView && !_this_.ifAnimate) {
        ...
        sliderLeft: function () {
            ...
            if (!this.ifAnimate) {
                ...
                if (this.currentSliderIndex == this.nImg - 1 * this.nView) {
                    this.moveType = -1;
        ...
        sliderAnimate: function () {
            ...
            Utils.animate(
                ...
                , {marginLeft: (this.viewwidth.substr(0, this.viewwidth.length - 2)-0+(this.marginLeftW+this.marginRightW)) * this.currentSliderIndex * (-1)-(this.marginLeftW+this.marginRightW) + 'px'}
                , _this_.speed
                , function () {
                    $(_this_.prevTagClassOrId).show();
                    $(_this_.nextTagClassOrId).show();
                    if (_this_.currentSliderIndex == 0) {
                        $( _this_.prevTagClassOrId).hide();
                    }
                    if (_this_.currentSliderIndex == _this_.nImg - 1 * _this_.nView) {
                        $(_this_.nextTagClassOrId).hide();
                    }
                    //因为currentSliderIndex是从0开始
                    $(_this_.rootTag + " .sliderWF " + _this_.currentIndexTagClassOrId).html(parseInt(_this_.currentSliderIndex) + 1);
                    _this_.ifAnimate = !_this_.ifAnimate; //解锁
